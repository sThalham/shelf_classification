...
# Path where augmented images get saved
path_out = "./images_augmented"

# Select the output image dimensions
DESIRED_IMG_HEIGHT = 168
DESIRED_IMG_WIDTH = 224

for filename in os.listdir(path_im):
    # Load images and masks
    img = cv2.imread(path_im + "/" + filename)
    if img is not None:
        segmentations = np.load(path_mask + "/" + filename + ".npy.npz")['arr_0']
        ...
        cv2.imwrite(path_out + "/" + filename, img)
