...
def main(args):
    ...
    for i, im_name in enumerate(im_list):
        ...
        if segms is None:
            segmentations = np.full((480, 640, 1), False)
        else:
            segmentations = mask_utils.decode(segms)

        #print(args.output_dir)
        #print(im_name.rsplit('/', 1)[1])
        np.savez_compressed(args.output_dir + '/' + im_name.rsplit('/', 1)[1] + '.npy', segmentations.astype(np.bool_))
        ...
        # np.save(args.output_dir + '/' + im_name.rsplit('/', 1)[1] + '_B.npy', segmentations)
